...
test("mip-5-verifications", async () => {
    ...
    try {
        ...
        // Go transfer MFAM to the deployer key from the F_MOVR_GRANT treasury, delegate those well to the deployer,
        // and assert the deployer has enough voting power to pass a proposal
        await setupDeployerAndEnvForGovernance(contracts, provider, F_MOVR_GRANT, FORK_BLOCK)
